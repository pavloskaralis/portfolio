...
function Text1(){
  ...
  return (
    <span>
      Traveler is designed to provide users with a platform for creating and planning trip itineraries with friends. Once created, an itinerary deploys two types of tables: the planning table where a group can input and gage interest in an activity, and the scheduling table, which renders for each trip date and allows users to sequence their activities. The app’s backend is created with Ruby on Rails, and employs PostgreSQL for its database, while the frontend is exemplary of an <a href='https://github.com/pavloskaralis/traveler/blob/master/traveler_frontend/package.json'>advanced React application</a>, as it uses React-Router-Dom, Redux-Thunk, and Styled Components to build a dynamic and maintainable structure.
      <br/><br/>
