...
function Text5(){
  ...
  return (
    <span>
      Pushing the limits of supported browser behavior by mixing text fields with interactive elements, Spell Check It serves as a phonetically oriented spell-checking API. The backend is written in Java using the Spring Boot framework, and operates by employing Soundex and <a target="_blank" href='https://github.com/pavloskaralis/SBA/blob/master/sba-api/src/main/java/main/Levenshtein.java'>Levenshtein</a> functions within its queries to a MySQL database. On the frontend, Angular is used to structure the client into 6 components, and users have the ability to toggle a dropdown menu of suggestions by clicking on a word. Once submitted, a user’s decision is returned to the server to accumulate data on common misspellings. 
      <br/><br/>
